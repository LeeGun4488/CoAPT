...
class MultiModalPromptLearner(nn.Module):
    ...
    def forward(self):
        ...
        prompts1 = self.construct_prompts(ctx, prefix, suffix1)
        prompts2 = self.construct_prompts(ctx, prefix, suffix2)
        prompts3 = self.construct_prompts(ctx, prefix, suffix3)
        ...
        return prompts, prompts1, prompts2, prompts3, self.proj(self.ctx), self.compound_prompts_text, visual_deep_prompts   # pass here original, as for visual 768 is required
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image, label=None):
        ...
        prompts, prompts1, prompts2, prompts3, shared_ctx, deep_compound_prompts_text, deep_compound_prompts_vision = self.prompt_learner()
        ...
        if self.prompt_learner.training:
            ...
            if self.subsample_classes == 'base':
                ...
                text_features1 = self.text_encoder(prompts1, tokenized_prompts1, deep_compound_prompts_text)
                text_features1 = text_features1 / text_features1.norm(dim=-1, keepdim=True)
                
                text_features2 = self.text_encoder(prompts2, tokenized_prompts2, deep_compound_prompts_text)
                text_features2 = text_features2 / text_features2.norm(dim=-1, keepdim=True)
                
                text_features3 = self.text_encoder(prompts3, tokenized_prompts3, deep_compound_prompts_text)
